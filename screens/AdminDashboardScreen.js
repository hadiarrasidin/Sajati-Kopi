...
export default function AdminDashboardScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data: ord, error: ordError } = await supabase.from('orders').select('*').order('id', { ascending: false });
      ...
      Alert.alert('Gagal', 'Tidak dapat memuat data. Periksa koneksi Supabase Anda.');
    } finally {
      setLoading(false);
    }
  };
